...
cdef class Node(object):
  ...
  def compute_best_response(self, bucket_seq_sb_, bucket_seq_bb_, best_response_to_sb):
    cdef:
      np.ndarray[int, ndim=1, cast=True] bucket_seq_sb = bucket_seq_sb_.astype(np.int32)
      np.ndarray[int, ndim=1, cast=True] bucket_seq_bb = bucket_seq_bb_.astype(np.int32)
      np.ndarray[float, ndim=1] util_sb = np.zeros(1, dtype=np.float32)
      ...
      int* bucket_seq_sb_ptr = <int*> bucket_seq_sb.data
      int* bucket_seq_bb_ptr = <int*> bucket_seq_bb.data
    self.best_response(1, 1, util_sb_ptr, util_bb_ptr, bucket_seq_sb_ptr, bucket_seq_bb_ptr, best_response_to_sb)
    return util_sb[0], util_bb[0]
        
  cdef void best_response(self, float p_sb, float p_bb, float* util_sb, float* util_bb,
                          int* bucket_seq_sb, int* bucket_seq_bb, bint best_response_to_sb):
    pass
...
cdef class RoundNode(Node):
  ...
  cdef void best_response(self, float p_sb, float p_bb, float* util_sb, float* util_bb,
    ...
    cdef:
      Node node = <Node> self.child_nodes[0]
    node.best_response(p_sb, p_bb, util_sb, util_bb, bucket_seq_sb, bucket_seq_bb, best_response_to_sb)
...
cdef class PlayerNode(Node):
  ...
  cdef void compute_prob(self, double* result, int node_bucket_times_num_child, float weight):
    ...
    for i in range(0, self.num_child):
      self.average_prob_ptr[node_bucket_times_num_child + i] = self.average_prob_ptr[node_bucket_times_num_child + i] + weight * result[i]

  cdef void compute_prob_no_update(self, double* result, int node_bucket_times_num_child, float weight):
    cdef double sum_regret_plus = 0
    cdef int i
    for i in range(0, self.num_child):
      result[i] = max(self.regret_ptr[node_bucket_times_num_child + i], 0)
  ...
  cdef void best_response(self, float p_sb, float p_bb, float* util_sb, float* util_bb,
    ...
    if self.is_sb:
      node_bucket = bucket_seq_sb[self.num_round]
      if best_response_to_sb:
        self.compute_prob_no_update(act_prob, node_bucket*self.num_child, p_sb)
        for i in range(0, self.num_child):
          node = <Node> self.child_nodes[i]
          node.best_response(p_sb * act_prob[i], p_bb, 
                             util_sb_child + i, util_bb_child + i, bucket_seq_sb, bucket_seq_bb,
                             best_response_to_sb)        
        for i in range(0, self.num_child):
          util_sb[0] += act_prob[i] * util_sb_child[i]
      ...
      if best_response_to_sb:
        ...
        for i in range(0, self.num_child):
          node = <Node> self.child_nodes[i]
          node.best_response(p_sb, p_bb * act_prob[i], 
                             util_sb_child + i, util_bb_child + i, bucket_seq_sb, bucket_seq_bb,
                             best_response_to_sb)
        for i in range(0, self.num_child):
          util_sb[0] += act_prob[i] * util_sb_child[i]
        ...
        self.compute_prob_no_update(act_prob, node_bucket*self.num_child, p_bb)
        for i in range(0, self.num_child):
          node = <Node> self.child_nodes[i]
          node.best_response(p_sb, p_bb * act_prob[i], 
                             util_sb_child + i, util_bb_child + i, bucket_seq_sb, bucket_seq_bb,
