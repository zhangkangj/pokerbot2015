...
class MixedtunedBot(base_bot.BaseBot):

# [0.625, 15.5, 0.5, 0.5]
# [0.625, 15.5, 0.5, 0.625]
  def __init__(self, player):
    super(MixedtunedBot, self).__init__(player)
    (self.mid_band_factor, self.preflop_raise_lim, self.flop_mid_card_lim, self.river_mid_card_lim) = 0.625, 15.5, 0.5, 0.625
    print "-----init:" + str([self.mid_band_factor, self.preflop_raise_lim, self.flop_mid_card_lim, self.river_mid_card_lim])
  ...
  def action(self):
    ...
    return super(MixedtunedBot, self).action(equity*0.8, can_raise, can_bet, can_call)
  ...
  def preflop(self, equity, can_raise, can_bet, can_call):
    ...
    if self.player.pot_size <= self.preflop_raise_lim:
        if is_raise_hand:
            if can_bet:
  ...
  def mid_card(self, equity, can_raise, can_bet, can_call):
    result = self.action_equity(equity, can_raise, can_bet, can_call, self.mid_band_factor);
    return result
  ...
  def flop(self, equity, can_raise, can_bet, can_call):
    #result = 'CHECK'
    min_to_bet = self.flop_mid_card_lim
    min_to_bet_1 = 0.75
  ...
  def river(self, equity, can_raise, can_bet, can_call):
    #result = 'CHECK'
    min_to_bet = self.river_mid_card_lim
    min_to_bet_1 = 0.75
