...
import mixedoppnew9_bot

import mixedoppnew9_opponent

class Mixedoppnew9Player(base_player.BasePlayer):
  def __init__(self):
    super(Mixedoppnew9Player, self).__init__()
    self.current_bot = mixedoppnew9_bot.Mixedoppnew9Bot(self)
  
  def new_game(self, parts):
    super(Mixedoppnew9Player, self).new_game(parts)
    
  def new_hand(self, parts):
    super(Mixedoppnew9Player, self).new_hand(parts)
    
  def handover(self, parts):
    super(Mixedoppnew9Player, self).handover(parts)

  def create_opponents(self):
    #debug
    print "-------------###--Mixedoppnew9Player.create_opponents()"
  
    # create a 'opponent model' 'opp0' for the player itself to store info for convenience
    self.opp0 = mixedoppnew9_opponent.Mixedoppnew9Opponent(self.player_name, self);
    # create models for the actual opponents
    self.opp1 = mixedoppnew9_opponent.Mixedoppnew9Opponent(self.opp1_name, self);
    self.opp2 = mixedoppnew9_opponent.Mixedoppnew9Opponent(self.opp2_name, self);
