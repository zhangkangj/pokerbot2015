...
class MixedparamBot(base_bot.BaseBot):
  ...
  def action(self):
    ...
    return super(MixedparamBot, self).action(equity*0.8, can_raise, can_bet, can_call)
  ...
  def preflop(self, equity, can_raise, can_bet, can_call):
    ...
    if self.player.pot_size <= self.player.param.preflop_raise_lim:
        if is_raise_hand:
            if can_bet:
  ...
  def mid_card(self, equity, can_raise, can_bet, can_call):
    result = self.action_equity(equity, can_raise, can_bet, can_call, self.player.param.mid_band_factor);
    return result
  ...
  def flop(self, equity, can_raise, can_bet, can_call):
    #result = 'CHECK'
    min_to_bet = self.player.param.flop_mid_card_lim
    min_to_bet_1 = 0.75
  ...
  def river(self, equity, can_raise, can_bet, can_call):
    #result = 'CHECK'
    min_to_bet = self.player.param.river_mid_card_lim
    min_to_bet_1 = 0.75
