...
import mixedoppnew8_bot

import mixedoppnew8_opponent

class Mixedoppnew8Player(base_player.BasePlayer):
  def __init__(self):
    super(Mixedoppnew8Player, self).__init__()
    self.current_bot = mixedoppnew8_bot.Mixedoppnew8Bot(self)
    self.param = None
  ...
  def new_game(self, parts):
    super(Mixedoppnew8Player, self).new_game(parts)
    
  def new_hand(self, parts):
    super(Mixedoppnew8Player, self).new_hand(parts)
    
  def handover(self, parts):
    super(Mixedoppnew8Player, self).handover(parts)

  def create_opponents(self):
    #debug
    print "-------------###--Mixedoppnew8Player.create_opponents()"
  
    # create a 'opponent model' 'opp0' for the player itself to store info for convenience
    self.opp0 = mixedoppnew8_opponent.Mixedoppnew8Opponent(self.player_name, self);
    # create models for the actual opponents
    self.opp1 = mixedoppnew8_opponent.Mixedoppnew8Opponent(self.opp1_name, self);
    self.opp2 = mixedoppnew8_opponent.Mixedoppnew8Opponent(self.opp2_name, self);
