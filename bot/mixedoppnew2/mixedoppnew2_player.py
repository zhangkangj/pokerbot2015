...
import mixedoppnew2_bot

import mixedoppnew2_opponent

class Mixedoppnew2Player(base_player.BasePlayer):
  def __init__(self):
    super(Mixedoppnew2Player, self).__init__()
    self.current_bot = mixedoppnew2_bot.Mixedoppnew2Bot(self)
  
  def new_game(self, parts):
    super(Mixedoppnew2Player, self).new_game(parts)
    
  def new_hand(self, parts):
    super(Mixedoppnew2Player, self).new_hand(parts)
    
  def handover(self, parts):
    super(Mixedoppnew2Player, self).handover(parts)

  def create_opponents(self):
    #debug
    print "-------------###--Mixedoppnew2Player.create_opponents()"
    self.opp1 = mixedoppnew2_opponent.Mixedoppnew2Opponent(self.opp1_name, self);
    self.opp2 = mixedoppnew2_opponent.Mixedoppnew2Opponent(self.opp2_name, self);
