...
import mixedoppnew7_bot

import mixedoppnew7_opponent

class Mixedoppnew7Player(base_player.BasePlayer):
  def __init__(self):
    super(Mixedoppnew7Player, self).__init__()
    self.current_bot = mixedoppnew7_bot.Mixedoppnew7Bot(self)
    self.param = None
  ...
  def new_game(self, parts):
    super(Mixedoppnew7Player, self).new_game(parts)
    
  def new_hand(self, parts):
    super(Mixedoppnew7Player, self).new_hand(parts)
    
  def handover(self, parts):
    super(Mixedoppnew7Player, self).handover(parts)

  def create_opponents(self):
    #debug
    print "-------------###--Mixedoppnew7Player.create_opponents()"
  
    # create a 'opponent model' 'opp0' for the player itself to store info for convenience
    self.opp0 = mixedoppnew7_opponent.Mixedoppnew7Opponent(self.player_name, self);
    # create models for the actual opponents
    self.opp1 = mixedoppnew7_opponent.Mixedoppnew7Opponent(self.opp1_name, self);
    self.opp2 = mixedoppnew7_opponent.Mixedoppnew7Opponent(self.opp2_name, self);
