...
import mixedoppnew5_bot

import mixedoppnew5_opponent

class Mixedoppnew5Player(base_player.BasePlayer):
  def __init__(self):
    super(Mixedoppnew5Player, self).__init__()
    self.current_bot = mixedoppnew5_bot.Mixedoppnew5Bot(self)
  
  def new_game(self, parts):
    super(Mixedoppnew5Player, self).new_game(parts)
    
  def new_hand(self, parts):
    super(Mixedoppnew5Player, self).new_hand(parts)
    
  def handover(self, parts):
    super(Mixedoppnew5Player, self).handover(parts)

  def create_opponents(self):
    #debug
    print "-------------###--Mixedoppnew5Player.create_opponents()"
  
    # create a 'opponent model' 'opp0' for the player itself to store info for convenience
    self.opp0 = mixedoppnew5_opponent.Mixedoppnew5Opponent(self.player_name, self);
    # create models for the actual opponents
    self.opp1 = mixedoppnew5_opponent.Mixedoppnew5Opponent(self.opp1_name, self);
    self.opp2 = mixedoppnew5_opponent.Mixedoppnew5Opponent(self.opp2_name, self);
