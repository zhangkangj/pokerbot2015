...
class TightAggressiveBot(base_bot.BaseBot):
  ...
  def cal_equity(self, bucket):
    equity = 0
    var = 0
    if self.player.num_board_cards == 3:
      if bucket <=11:
        ...
        equity = 0.45
        var = 0.2
    elif self.player.num_board_cards == 4:
      if bucket <= 18:
  ...
  def preflop(self, can_raise, can_bet, can_call):
    ...
    num_raise = num_of_raise(self.last_actions_preflop)
    decay = decay_factor ** num_raise
        
    prob_arr = np.load('data/evaluator/preflop_prob.npy')
    hole_card_idx = preflop_idx(self.player.hole_cards)
    prob = prob_arr[hole_card_idx]
  ...
  def flop(self, can_raise, can_bet, can_call):
    ...
    buckets =np.load('data/evaluator/flop_bucket.npy')
    bucket = buckets[idx]
    
    equity = cal_equity(bucket)    
  ...
  def turn(self, can_raise, can_bet, can_call):
    ...
    idx = evaluator.flop_index(hole_cards[0],hole_cards[1],board_cards[0],board_cards[1],board_cards[2])
    buckets =np.load('data/evaluator/turn_bucket.npy')
    bucket = buckets[idx]
